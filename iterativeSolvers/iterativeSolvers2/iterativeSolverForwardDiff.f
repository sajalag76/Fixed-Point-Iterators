      ...
      double precision A(4,4), y,yb,yd,B(4),Bd(4),Bb(4),xOld(4),delta
	  double precision ydelta,derivative
      integer::j,i
...
c      xOld=(/0.6 ,2.2727 ,-1.1 ,1.875/)
	  ...
	  do j=1,4
	     ...
	     ydelta=0.0
	     A(i,j)=A(i,j)+delta
		 call func(A,B,xOld,ydelta);
		 derivative=(ydelta-y)/delta;
		 print *,i,j,derivative
	     A(i,j)=A(i,j)-delta
          end do
          end do
...
c	   y = 1*xOld(1) + 2*xOld(2) + 3*xOld(3) + 4*xOld(4)
      end subroutine obj
