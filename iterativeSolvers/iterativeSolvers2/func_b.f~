...
C   with respect to varying inputs: y a
C   RW status of diff variables: y:in-zero a:out
C
      SUBROUTINE FUNC_B(a, ab, b, xold, y, yb)
      IMPLICIT NONE
      DOUBLE PRECISION a(4, 4), b(4), xold(4), y, xnew(4), temp, ep
      DOUBLE PRECISION ab(4, 4), xoldb(4), yb, xnewb(4), tempb
 ...
 110    DO j=1,4
      ...
      DO i0=1,ad_count
        ...
        DO i=4,1,-1
          ...
          tempb = -tempb0
          ab(i, i) = ab(i, i) - (b(i)-temp)*tempb0/a(i, i)
          xnewb(i) = 0.D0
          DO j=4,1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) THEN
              ab(i, j) = ab(i, j) + xold(j)*tempb
              xoldb(j) = xoldb(j) + a(i, j)*tempb
            END IF
          ENDDO
        ENDDO
      ENDDO
